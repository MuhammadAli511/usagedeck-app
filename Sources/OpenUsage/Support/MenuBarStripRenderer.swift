...
enum MenuBarStripRenderer {
    ...
    static let privacyImage: NSImage? = {
        ...
        image.isTemplate = true
        image.accessibilityDescription = "UsageDeck, usage hidden while the screen is shared"
        return image
    }()
    ...
    static let fallbackIcon: NSImage = {
        let image = NSImage(
            ...
            accessibilityDescription: "UsageDeck"
        ) ?? NSImage()
        image.isTemplate = true
        return image
    }()
...
private struct MenuBarPrivacyLabel: View {
    var body: some View {
        HStack(spacing: 5) {
            ...
            if let mark = ProviderMarks.mark(for: "usagedeck") {
