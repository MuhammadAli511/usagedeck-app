...
final class LogFileTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("UsageDeckTests.LogFile.\(UUID().uuidString)", isDirectory: true)
    }
    ...
    func testResolvedPathEndsWithExpectedSuffix() {
        XCTAssertTrue(
            LogFile.url.path.hasSuffix("Logs/UsageDeck/UsageDeck.log"),
    ...
    func testAppendCreatesFileAndWritesLine() throws {
        let log = LogFile(directory: tempDir, fileName: "UsageDeck.log")
        log.open()
        log.append("2026-01-01T00:00:00Z [INFO] [config] hello")

        let fileURL = tempDir.appendingPathComponent("UsageDeck.log")
    ...
    func testRotationCreatesBackupWhenCapExceeded() throws {
        ...
        let cap = 200
        let log = LogFile(directory: tempDir, fileName: "UsageDeck.log", maxBytes: cap)
        log.open()
        ...
        let mainURL = tempDir.appendingPathComponent("UsageDeck.log")
        let archiveURL = tempDir.appendingPathComponent("UsageDeck.1.log")
    ...
    func testRotationKeepsOnlyOneArchive() throws {
        ...
        let archiveURL = tempDir.appendingPathComponent("UsageDeck.1.log")
        let secondArchiveURL = tempDir.appendingPathComponent("UsageDeck.2.log")
