...
UsageDeck always sends an **anonymous daily active ping** and **anonymous crash reports** so we can
...
**Settings → Privacy → Help make UsageDeck better by sharing anonymous usage analytics**. Existing
installs keep the choice they already stored.

## What is always shared

Once per local day, UsageDeck sends an anonymous **app use** ping: that the app was active today, the
...
- **Crash reports** — if UsageDeck crashes, it saves a report and sends it the next time you open the
  app: the technical stack trace (which parts of *UsageDeck's own code* were running when it crashed)
...
When extra analytics are on, UsageDeck also sends, for each provider refreshed that day, at most one
...
UsageDeck primarily reads credentials that provider tools already keep on your Mac. When it writes a
...
Claude Desktop access is strictly read-only. UsageDeck may ask macOS for permission to use the
...
Besides the provider API calls the vendor's own tools would make, UsageDeck fetches public [model price lists](pricing.md) about once an hour (from `raw.githubusercontent.com`, `models.dev`, and this project's GitHub Pages). These are plain downloads of public data — they carry no usage, log, or account information, and they run regardless of the analytics toggle. The spend tiles are computed from local CLI logs entirely on your Mac; no log data ever leaves it.

To avoid re-reading unchanged Claude, Codex, and pi logs after every relaunch, UsageDeck keeps their
parsed usage events in `~/Library/Application Support/UsageDeck/log-scan-cache/`. These records contain
...
identity caches that have not been used for 35 days are removed. UsageDeck's pricing engine runs after
the cache is read, so its computed aggregates and totals are not persisted in this cache.

If you explicitly turn on [iCloud Sync](icloud-sync.md), UsageDeck writes normalized daily tokens,
...
- Data is fully anonymous: UsageDeck never identifies you to the analytics service and creates no user profile.
...
Open **Settings → Privacy** and switch **Help make UsageDeck better by sharing anonymous usage analytics**
