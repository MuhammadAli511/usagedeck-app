...
UsageDeck can route all provider requests through an optional proxy.

- Supported: `socks5://`, `http://`, `https://`
- Config file: `~/.usagedeck/config.json`
- Default: off
- UI: none — file only
...
- The config is read once at launch — **restart UsageDeck after changing the file**.
