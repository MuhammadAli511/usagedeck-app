...
`/Applications`. The dev build uses its own bundle id (`com.robinebers.usagedeck.dev`), so it keeps its
own settings and keychain and never disturbs a released UsageDeck. It ships no update feed, so it never
...
```sh
log stream --info --style compact --predicate 'process == "UsageDeck"'
```

To read logs *after the fact* instead of live, use `log show` with a time window:

```sh
log show --last 10m --info --predicate 'process == "UsageDeck"'
```

## Log file

In addition to the unified log above, the app writes a file log to
`~/Library/Logs/UsageDeck/UsageDeck.log` — this is what to send with a support report. It is capped at
