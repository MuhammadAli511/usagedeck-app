...
enable it in Customize. When Claude reports your plan name, UsageDeck shows it beside the provider name.
...
Sign in with Claude Code or Claude Desktop; UsageDeck reads the existing login. It checks these sources, preferring one that can read your subscription usage:
...
when needed. UsageDeck verifies that a credential belongs to the correct account and organization.

Claude Desktop support is read-only. UsageDeck decrypts its currently valid access token using the
`Claude Safe Storage` item in your macOS Keychain. It never reads or uses Desktop's refresh token, and
never changes Desktop's config, cookies, or Keychain entry. This prevents UsageDeck from invalidating
...
macOS asks once before UsageDeck can access that Keychain item. Background refreshes never open the
password dialog: UsageDeck first asks you to refresh manually, and choosing **Always Allow** makes later
refreshes silent. If Desktop's short-lived token expires, open Claude Desktop so it can renew the login,
then refresh UsageDeck.

A `CLAUDE_CODE_OAUTH_TOKEN` — usually a long-lived `claude setup-token` — can run the model but can't read your Session and Weekly limits, and it often lingers in your shell environment. So when a real keychain or file login is present, UsageDeck uses that login for the live meters and keeps the environment token only as a fallback; the Session/Weekly meters no longer go blank just because that token is set. If the environment token is your *only* credential (a headless setup), it's used on its own and the spend tiles still load from local logs.
...
or Desktop shares the same card when both the account and organization match. Restart UsageDeck after
...
separate default card alongside saved Swap accounts. UsageDeck does not guess which saved
...
a final, read-only fallback. UsageDeck does not rotate vault tokens or modify Claude Swap's account
list. If a vault login is stale, launch that account with `cswap run <email>`, then refresh UsageDeck.
...
Conductor, count as long as UsageDeck has never seen more than one Claude account. Once multiple
...
spend window. Sessions with conflicting account or organization records are excluded. UsageDeck
...
- **"Claude Desktop login is stale"** — open Claude Desktop so it can renew the login, then refresh UsageDeck.
...
- **"Updates blocked by Anthropic"** (an amber warning on the Claude header) — the usage API is throttling UsageDeck. It keeps the last values from the same login, shows when it will retry, and backs off in the meantime. A different login starts with a fresh cache and cooldown.
- **Spend tiles show "No data"** — UsageDeck found no Claude Code logs in the last 30 days. If your logs live somewhere custom, set `CLAUDE_CONFIG_DIR` so both Claude Code and UsageDeck look in the same place.

## Under the hood

`GET https://api.anthropic.com/api/oauth/usage` with the selected OAuth token. Claude Code tokens refresh via `platform.claude.com/v1/oauth/token`; Claude Desktop tokens are read-only and must be renewed by Desktop itself. If a token is expired or revoked, UsageDeck retries with the next credential source before reporting an error.
