...
When Z.ai reports your plan name, UsageDeck shows it beside the provider name.

## Where credentials come from

Z.ai has no companion CLI/app that UsageDeck can reuse a credential from, so you supply an API key.
UsageDeck reads it from the first place it finds one, in this order:

1. `~/.config/usagedeck/zai.json` — `{"apiKey":"…"}` (the file Settings writes to)
...
2. Add the key to UsageDeck via **Settings → API Keys**, **or** export it:
