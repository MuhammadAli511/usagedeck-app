...
UsageDeck shows the reported tier (such as "Pay as you go" or "Free tier") beside the provider name.
...
The key is stored at `~/.config/usagedeck/openrouter.json` and picked up on the next refresh.

You can also provide the key directly (checked in this order, first match wins):

1. **Config file:** `~/.config/usagedeck/openrouter.json` — the file the Settings card writes:
