...
How UsageDeck decides which providers start on, what happens when an update adds a new provider, and the one rule that governs it all: **your own toggles always win and are never overridden.**
...
The same detection runs for providers that arrive later. On the first launch after an update, UsageDeck compares the providers it now ships with the ones this install has seen before. For each brand-new one, it runs the same local-only credential check:
