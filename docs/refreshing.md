...
- The one-shot `usagedeck` command reuses this same persisted cache for five minutes, refreshes missing or stale entries without starting the app, and exits. `usagedeck --force` runs the same forced provider refresh as ⌘R regardless of cache age.
...
`~/Library/Application Support/UsageDeck/log-scan-cache/`. It stores parsed usage events before UsageDeck
