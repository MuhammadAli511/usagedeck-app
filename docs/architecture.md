...
A high-level map of how UsageDeck is put together, for people working on the code. For what the app
*does*, start with the [behavior docs](README.md).

## The shape of the app

UsageDeck is a SwiftPM package with a shared module and two thin executables — there is no Xcode project.
...
The `usagedeck` executable imports the same module. A normal invocation reads `ProviderSnapshotCache`
...
1. **Auth store** — reads credentials that already exist on the machine (config files, keychain). UsageDeck
...
macOS menu-bar apps live in an `NSStatusItem`. UsageDeck shows its content in a custom, key-capable
...
UsageDeck runs on macOS 15 (Sequoia) and later. It is built against the latest SDK and back-deploys:
