...
UsageDeck already lists these credits (the "Resets" surface on the Codex provider); this
...
enterprise/alternative base URLs; UsageDeck uses the ChatGPT style.

Headers on every call (identical to what UsageDeck's Codex usage client already sends):
...
### List (already implemented in UsageDeck)
...
## Implementation notes for UsageDeck (when we build it)

- The claim is a single POST on infrastructure UsageDeck already talks to; auth, headers,
