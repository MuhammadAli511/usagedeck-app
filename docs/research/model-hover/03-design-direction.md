...
**Question:** What should the hover panel for the Today / Yesterday / Last 30 Days spend rows look like, given the attached AI-generated concept (a "Models" flyout with a "TOP DRIVER" hero card, donut, and ranked model list)? The owner's verdict on the concept: directionally right, but "a bit over the top" — inconsistent type sizes, not native enough. Goal: an Apple-first, native macOS treatment that belongs in UsageDeck, works for Cursor / Claude / Codex / Grok, and is not Cursor-branded.
...
Read from `Sources/UsageDeck/Views/` + `Sources/UsageDeck/Stores/DensitySetting.swift` + `Sources/UsageDeck/Support/Theme.swift` + `Sources/UsageDeck/Support/LiquidGlassFallbacks.swift`. The new panel must match this.
...
| Footer identity ("UsageDeck 0.7.x", "Next update in 3m") | — | — | `.caption2` (semantic) | `.secondary` |
