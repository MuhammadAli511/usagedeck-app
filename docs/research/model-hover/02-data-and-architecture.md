...
> [`SpendTileMapper.swift`](../../../Sources/UsageDeck/Providers/SpendTileMapper.swift),
> [`HoverPopoverState.swift`](../../../Sources/UsageDeck/Views/HoverPopoverState.swift), and
> [`ModelUsageDetail.swift`](../../../Sources/UsageDeck/Views/ModelUsageDetail.swift) for the current
...
Shared spend tiles are produced in `Sources/UsageDeck/Providers/SpendTileMapper.swift`.
...
It consumes `DailyUsageSeries` from `Sources/UsageDeck/Models/DailyUsageSeries.swift`. That type is intentionally provider-neutral and contains only:
...
The shared pricing engine is in `Sources/UsageDeck/Pricing/`:
...
- `CursorUsageCSVRow` in `Sources/UsageDeck/Providers/Cursor/CursorUsageCSV.swift` carries:
  - `date: Date`
  - `model: String`
  - `maxMode: Bool`
...
- `ClaudeLogUsageScanner.Entry` in `Sources/UsageDeck/Providers/Claude/ClaudeLogUsageScanner.swift` carries:
  - `timestamp: Date`
  - `tokens: TokenBreakdown`
  - `costUSD: Double?`
  - `model: String?`
...
- `CodexLogUsageScanner.Event` in `Sources/UsageDeck/Providers/Codex/CodexLogUsageScanner.swift` carries:
  - `timestamp: Date`
  - `model: String`
  - `input: Int`
  - `cached: Int`
...
- `WidgetDescriptor.spendTiles(provider:)` in `Sources/UsageDeck/Models/WidgetDescriptor+Factories.swift` declares the three spend descriptors:
  - `<provider>.today`
  - `<provider>.yesterday`
  - `<provider>.last30`
...
- Implemented in `Sources/UsageDeck/Views/HoverTooltip.swift`.
...
- `UsageSparkline` in `Sources/UsageDeck/Views/UsageSparkline.swift` attaches hover only to the bar strip, not the whole row title.
- It uses `TrendHoverState` from `Sources/UsageDeck/Views/UsageTrendDetail.swift`:
  - 400ms reveal dwell
...
- Tests in `Tests/UsageDeckTests/UsageTrendTests.swift` cover the open/close/quick-pass behavior.
