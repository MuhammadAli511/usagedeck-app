...
> [`SpendTileMapper.swift`](../../../Sources/UsageDeck/Providers/SpendTileMapper.swift), and
> [`ModelUsageDetail.swift`](../../../Sources/UsageDeck/Views/ModelUsageDetail.swift) for current
...
1. **Product / UX uncertainty (owner)** — [Comment on #740](https://github.com/MuhammadAli511/usagedeck-app/pull/740#issuecomment-4802994002): works for Cursor and could extend to other providers, but *“I wonder if there is a better way to display this. Maybe as a chart or something? Not sure.”* That reads as dissatisfaction with the **leaderboard row + hover popover** pattern, not the underlying aggregation.
...
|-------------|----------------|
| `CursorPricing`, `CursorModelManifest`, bundled **`model_manifest.json`** | **Removed.** All imputation through **`Sources/UsageDeck/Pricing/`** (`ModelPricing`, `ModelPricingStore`, LiteLLM + models.dev + **`pricing_supplement.json`**) — see **`docs/pricing.md`**. |
...
- **Snapshot cache key:** #740 bumps to **`usagedeck.providerSnapshots.v7`** (model breakdown + variants). `main` is at **`v6`** (`.values` **`unknownModels`** on spend tiles) — different schema story; a revival would need a fresh bump and migration reasoning.
...
- **`Sources/UsageDeck/Resources/pricing_supplement.json`** is published to GitHub Pages; apps refresh ~daily without a release. #740’s approach of extending **`model_manifest.json`** for family metadata is obsolete; new models/aliases belong in the **supplement** and **`docs/pricing.md`** maintainer flow.
...
- PR: https://github.com/MuhammadAli511/usagedeck-app/pull/740  
- Pricing overhaul: https://github.com/MuhammadAli511/usagedeck-app/pull/827 (merged)  
- Current pricing doc: `docs/pricing.md`  
- Shared spend tiles: `Sources/UsageDeck/Providers/SpendTileMapper.swift`  
- Cursor CSV + day aggregation (model still dropped): `Sources/UsageDeck/Providers/Cursor/CursorUsageMapper.swift` (`appendSpendLines`)
