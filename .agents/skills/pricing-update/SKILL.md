...
Read `Sources/UsageDeck/Resources/pricing_supplement.json` and compare:
...
python3 - << 'PY'
import json, re, sys
with open("Sources/UsageDeck/Resources/pricing_supplement.json") as f:
    s = json.load(f)
problems = []
...
If a new alias rule maps a slug that appears in real usage, add a resolution test case in `Tests/UsageDeckTests/ModelPricingTests.swift`.
...
curl -s https://muhammadali511.github.io/usagedeck-app/pricing_supplement.json | python3 -c "import json,sys; print(json.load(sys.stdin)['updated_at'])"
```
