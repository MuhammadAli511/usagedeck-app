...
UsageDeck is a SwiftPM-based SwiftUI menu-bar app for macOS that shows AI provider usage widgets (Claude, Codex, Cursor, Grok, Devin, and more).
...
> **Repository note:** This is the native Swift edition of UsageDeck. Active development happens on the `main` branch. (NOT the legacy Tauri version which now sits in the `tauri-legacy` branch)
...
Conventions for the per-provider modules under `Sources/UsageDeck/Providers/<Name>/`.
...
- **Model pricing:** all spend imputation (Claude, Codex, Cursor, Grok) prices through the shared engine in `Sources/UsageDeck/Pricing/` (see `docs/pricing.md`). Cursor-native model rates and alias rules live in `Sources/UsageDeck/Resources/pricing_supplement.json` — sync new or changed models from [Cursor models & pricing](https://cursor.com/docs/models-and-pricing.md) (update `updated_at`, pricing entries, and `alias_rules` for CSV model slugs); merging to `main` publishes it to gh-pages, so installed apps pick it up without a release. The bundled LiteLLM/models.dev snapshots regenerate with `script/update_pricing_snapshots.sh` (a release-time chore).
...
- Match the existing design language; UsageDeck has a specific look and feel.
