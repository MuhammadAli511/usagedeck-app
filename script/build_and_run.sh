...
elif [ -f "$PREBUILT_ICON_DIR/Assets.car" ] || [ -f "$PREBUILT_ICON_DIR/AppIcon.icns" ]; then
  # actool crashes on Icon Composer sources from Xcode 26.4 onward (FB20183399), so assets/AppIcon.prebuilt
  # ships a prebuilt icon that release CI and this dev build both fall back to. Either artifact alone is
  # enough: the .icns is generated by script/make_icns.py and needs only iconutil.
